...
parser.add_argument('--Q', type=str, default='',
					help='Question number. If 5.1 or 5.2 or 5.3, will load model.')
...
###Q5 don't make extra directory for Q5
if args.Q is '':
...
def run_epoch(model, data, is_train=False, lr=1.0):
	...
	###Q5.1 begin
	if (args.Q == '5.1') and (is_train is False):
		mean_loss_t /= len(data)
		np.save('{}_losst.npy'.format(load_path.split("_")[0]),mean_loss_t)
	### end #################################

	return np.exp(costs / iters), losses

###Q5.1
def run_losst(model, data):
	model.eval()
	epoch_size = ((len(data) // model.batch_size) - 1) // model.seq_len
	start_time = time.time()
	mean_loss_t = None
	...
	for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
		...
		for t in range(timesteps):
			loss_t.append(loss_fn(outputs[t,:,:],targets[t,:]).data.cpu().numpy())
		loss_t = np.array(loss_t)

		if mean_loss_t is None:
			mean_loss_t = loss_t
		else:
			mean_loss_t += loss_t
...
def run_hgrads(model, data):
	...
	# TAKES FIRST MINIBATCH FROM TRAINING SET
	data_iterator = ptb_iterator(data, model.batch_size, model.seq_len)
	(x, y) = next(data_iterator)
	inputs = torch.from_numpy(x.astype(np.int64)).transpose(0, 1).contiguous().to(device)#.cuda()
	targets = torch.from_numpy(y.astype(np.int64)).transpose(0, 1).contiguous().to(device)#.cuda()

	model.zero_grad()
	outputs, hidden, hiddens = model.hgrads(inputs, hidden)

	timesteps = outputs.shape[0]
	final_t_loss = loss_fn(outputs[-1, :, :], targets[-1, :])
	hgrads = torch.autograd.grad(final_t_loss, hiddens, retain_graph=True)
	hgrads = {'h%d'%(i+1):torch.cat([hgrads[2*i], hgrads[(2*i)+1]]) for i in range(35)}
	return hgrads

###Q5.3
def run_generation(model, data):
	model.eval()
	start_time = time.time()
	hidden = model.init_hidden()
	hidden = hidden.to(device)

	# TAKES FIRST MINIBATCH FROM VALIDATION SET
	data_iterator = ptb_iterator(data, model.batch_size, model.seq_len)
	(x, y) = next(data_iterator)

	# TAKES FIRST WORD FROM MINIBATCH SAMPLES
	inputs = torch.from_numpy(x.astype(np.int64)).transpose(0, 1).contiguous().to(device)#.cuda() # shape: (seq_len, batch_size)
	input = inputs[0]

	model.zero_grad()
	hidden = repackage_hidden(hidden)
	generations = model.generate(input, hidden, args.seq_len) # shape: (generated_seq_len, batch_size)

	generations = generations.cpu().numpy().T
	generated_sentences = []
	for generation in generations:
		generated_sentences.append(' '.join(id_2_word[id] for id in generation))
